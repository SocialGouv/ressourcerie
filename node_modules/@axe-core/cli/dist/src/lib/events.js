...
exports.default = ({ silentMode, timer, cliReporter, verbose, exit }) => {
    return {
        ...
        onTestStart: (url) => {
            ...
            console.log(utils_1.bold('\nTesting ' + utils_1.link(url)) +
        ...
        onTestComplete: (results) => {
            ...
            if (violations.length === 0) {
                cliReporter(utils_1.green(' 0 violations found!'));
                return;
            }
            const issueCount = violations.reduce((count, violation) => {
                cliReporter('\n' +
                    utils_1.error('  Violation of %j with %d occurrences!\n') +
                    '    %s. Correct invalid elements at:\n' +
                    violation.nodes
                        .map(node => '     - ' + utils_1.selectorToString(node.target) + '\n')
                        .join('') +
                    '    For details, see: %s', violation.id, violation.nodes.length, violation.description, utils_1.link(violation.helpUrl.split('?')[0]));
                return count + violation.nodes.length;
            }, 0);
            cliReporter(utils_1.error('\n%d Accessibility issues detected.'), issueCount);
            if (verbose) {
                const metadata = {

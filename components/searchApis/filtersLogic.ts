...
export const filterKind = (selectedKind: string | null) => {
  if (!selectedKind) {
    return () => true;
  }
  return (api: IApi) => api.kind.includes(selectedKind);
};

export const filterProducer = (selectedProducer: string | null) => {
  if (!selectedProducer) {
    return () => true;
  }
  return (api: IApi) => api.producer.includes(selectedProducer);
};
